...
const JournalTeenInnovation = () => {
  const navigate = useNavigate();
  const location = useLocation();
  
  // Get game data from game category folder (source of truth)
  const gameId = "ehe-teen-37";
  ...
  const stages = [
    {
      question: 'Write: "One problem I want to solve is ___.',
      minLength: 10,
    },
    {
      question: 'Write: "This problem affects people because ___.',
      minLength: 10,
    },
    {
      question: 'Write: "My innovative solution would ___.',
      minLength: 10,
    },
    {
      question: 'Write: "To create this solution, I need ___.',
      minLength: 10,
    },
    {
      question: 'Write: "This innovation could change lives by ___.',
      minLength: 10,
    },
  ];
  ...
  useEffect(() => {
    if (showResult) {
      console.log(`🎮 Journal of Teen Innovation game completed! Score: ${finalScore}/${stages.length}, gameId: ${gameId}`);
    }
  }, [showResult, finalScore, gameId, stages.length]);

  const handleNext = () => {
    navigate("/student/ehe/teens/simulation-prototype-test");
  };

  return (
    <GameShell
      title="Journal of Teen Innovation"
      subtitle={!showResult ? `Question ${currentStage + 1} of ${stages.length}: Solve problems creatively!` : "Journal Complete!"}
      ...
      nextGamePathProp="/student/ehe/teens/simulation-prototype-test"
      nextGameIdProp="ehe-teen-38">
      <div className="min-h-[calc(100vh-200px)] flex flex-col justify-center text-center text-white space-y-6 md:space-y-8 max-w-4xl mx-auto px-4 py-4">
        ...
        {showResult && (
          <div className="bg-white/10 backdrop-blur-md p-6 md:p-8 rounded-xl md:rounded-2xl border border-white/20 text-center">
            ...
            <h2 className="text-2xl font-bold text-white mb-6">Innovation Challenge Complete!</h2>
            
            <div className="bg-gradient-to-r from-green-500/20 to-emerald-500/20 rounded-xl p-4 border border-green-400/30 mb-6">
              <p className="text-green-300 font-bold">
                🎉 Excellent! You've completed your innovation journal entry!
              </p>
              <p className="text-green-300 mt-2">
                Identifying problems is the first step toward creating innovative solutions!
              </p>
            </div>
...
export default JournalTeenInnovation;