...
const JournalOfIdeas = () => {
  const navigate = useNavigate();
  const location = useLocation();
  
  // Get game data from game category folder (source of truth)
  const gameId = "ehe-kids-37";
  ...
  const stages = [
    {
      question: 'Write: "One idea I have to solve a problem is ___.',
      minLength: 10,
    },
    {
      question: 'Write: "This idea will help people by ___.',
      minLength: 10,
    },
    {
      question: 'Write: "I can test my idea by ___.',
      minLength: 10,
    },
    {
      question: 'Write: "A role model who had great ideas is ___.',
      minLength: 10,
    },
    {
      question: 'Write: "My dream invention would be ___.',
      minLength: 10,
    },
  ];
  ...
  useEffect(() => {
    if (showResult) {
      console.log(`🎮 Journal of Ideas game completed! Score: ${finalScore}/${stages.length}, gameId: ${gameId}`);
    }
  ...
  return (
    <GameShell
      ...
      subtitle={!showResult ? `Question ${currentStage + 1} of ${stages.length}: Innovate and create!` : "Journal Complete!"}
      ...
      nextGamePathProp="/student/ehe/kids/game-story"
      nextGameIdProp="ehe-kids-38">
      <div className="min-h-[calc(100vh-200px)] flex flex-col justify-center text-center text-white space-y-6 md:space-y-8 max-w-4xl mx-auto px-4 py-4">
        ...
        {showResult && (
          <div className="bg-white/10 backdrop-blur-md p-6 md:p-8 rounded-xl md:rounded-2xl border border-white/20 text-center">
            ...
            <h2 className="text-2xl font-bold text-white mb-6">Innovation Complete!</h2>
            
            <div className="bg-gradient-to-r from-green-500/20 to-emerald-500/20 rounded-xl p-4 border border-green-400/30 mb-6">
              <p className="text-green-300 font-bold">
                🎉 Excellent! You've completed your ideas journal entry!
              </p>
              <p className="text-green-300 mt-2">
                Recording your ideas helps develop your innovation skills!
              </p>
            </div>
