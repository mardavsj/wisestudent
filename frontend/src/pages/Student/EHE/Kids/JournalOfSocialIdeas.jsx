...
const JournalOfSocialIdeas = () => {
  const navigate = useNavigate();
  const location = useLocation();
  
  // Get game data from game category folder (source of truth)
  const gameId = "ehe-kids-87";
  ...
  const stages = [
    ...
    {
      question: 'Write: "My solution to this problem is ___.',
      minLength: 10,
    },
    {
      question: 'Write: "To test my solution, I would ___.',
      minLength: 10,
    },
    {
      question: 'Write: "My invention to solve this would be ___.',
      minLength: 10,
    },
  ];
  ...
  useEffect(() => {
    if (showResult) {
      console.log(`🎮 Journal of Social Ideas game completed! Score: ${finalScore}/${stages.length}, gameId: ${gameId}`);
    }
  ...
  return (
    <GameShell
      ...
      subtitle={!showResult ? `Question ${currentStage + 1} of ${stages.length}: Solve real-world problems!` : "Journal Complete!"}
      ...
      <div className="min-h-[calc(100vh-200px)] flex flex-col justify-center text-center text-white space-y-6 md:space-y-8 max-w-4xl mx-auto px-4 py-4">
        ...
        {showResult && (
          <div className="bg-white/10 backdrop-blur-md p-6 md:p-8 rounded-xl md:rounded-2xl border border-white/20 text-center">
            ...
            <h2 className="text-2xl font-bold text-white mb-6">Problem Solving Complete!</h2>
            
            <div className="bg-gradient-to-r from-green-500/20 to-emerald-500/20 rounded-xl p-4 border border-green-400/30 mb-6">
              <p className="text-green-300 font-bold">
                🎉 Excellent! You've completed your social ideas journal entry!
              </p>
              <p className="text-green-300 mt-2">
                Identifying problems is the first step to creating solutions!
              </p>
            </div>
...
export default JournalOfSocialIdeas;