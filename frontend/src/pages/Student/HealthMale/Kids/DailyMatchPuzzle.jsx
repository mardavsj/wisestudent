...
const DailyMatchPuzzle = () => {
  ...
  const correctMatches = [
    { activityId: 1, benefitId: 1 }, // Sleep → Energy Boost
    { activityId: 2, benefitId: 2 }, // Healthy Food → Strong Muscles
    { activityId: 3, benefitId: 3 }, // Exercise → Heart Health
    { activityId: 4, benefitId: 4 }, // Reading → Brain Power
    { activityId: 5, benefitId: 5 }  // Bathing → Germs Removed
  ];
  ...
  return (
    <GameShell
      ...
      subtitle={gameFinished ? "Puzzle Complete!" : `Match Activities with Benefits (${matches.length}/${activities.length} matched)`}
      onNext={handleNext}
            ...
            {score >= 3 ? (
              <div>
                ...
                <p className="text-white/90 text-lg mb-4">
                  You correctly matched {score} out of {activities.length} daily activities with their benefits!
                </p>
                <div className="bg-gradient-to-r from-yellow-500 to-orange-500 text-white py-3 px-6 rounded-full inline-flex items-center gap-2 mb-4">
                  <span>+{score} Coins</span>
                </div>
                <p className="text-white/80">
                  Lesson: Doing healthy activities every day helps your body and mind grow strong!
                </p>
              </div>
            ) : (
              <div>
                ...
                <p className="text-white/80 text-sm">
                  Tip: Think about how each activity helps your body or mind!
                </p>
              </div>
            )}
          </div>
