...
const DangerousRobotStoryy = () => {
  ...
  const { nextGamePath, nextGameId } = useMemo(() => {
    ...
    try {
      ...
      if (currentGame && currentGame.index !== undefined) {
        ...
        return {
          nextGamePath: nextGame ? nextGame.path : "/student/ai-for-all/teen/good-vs-bad-data-puzzle",
    ...
    return { nextGamePath: "/student/ai-for-all/teen/good-vs-bad-data-puzzle", nextGameId: null };
  ...
  useEffect(() => {
    if (showResult) {
      console.log(`🎮 Dangerous Robot Story game completed! Score: ${finalScore}/${questions.length}, gameId: ${gameId}, nextGamePath: ${nextGamePath}, nextGameId: ${nextGameId}`);
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-xl md:rounded-2xl p-6 md:p-8 border border-white/20 text-center flex-1 flex flex-col justify-center">
            {finalScore >= 3 ? (
              <div>
                ...
                <h3 className="text-xl md:text-2xl font-bold text-white mb-4">Safe Choice!</h3>
                <p className="text-white/90 text-base md:text-lg mb-4">
                  You got {finalScore} out of {questions.length} questions correct!
                  Excellent! Turning off dangerous robots prevents harm and keeps everyone safe.
                </p>
                <div className="bg-gradient-to-r from-yellow-500 to-orange-500 text-white py-2 md:py-3 px-4 md:px-6 rounded-full inline-flex items-center gap-2 mb-4 text-sm md:text-base">
                  <span>+{coins} Coins</span>
                </div>
                <p className="text-white/80 text-sm md:text-base">
                  Great work! 🧠 You understand how to respond to dangerous AI situations. Safety first with robotics! 🚀
                </p>
              </div>
            ) : (
              <div>
                ...
                <p className="text-white/90 text-base md:text-lg mb-4">
                  You got {finalScore} out of {questions.length} questions correct.
                  When robots become dangerous, the safest action is to disable them immediately.
                </p>
                <button
                ...
                <p className="text-white/80 text-xs md:text-sm">
                  Try again to reinforce your understanding of robot safety protocols!
                </p>
              </div>
            )}
          </div>
