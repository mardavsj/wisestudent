...
const BadDataStory = () => {
  ...
  const { nextGamePath, nextGameId } = useMemo(() => {
    ...
    try {
      ...
      if (currentGame && currentGame.index !== undefined) {
        ...
        return {
          nextGamePath: nextGame ? nextGame.path : "/student/ai-for-all/teen/missing-data-puzzlee",
    ...
    return { nextGamePath: "/student/ai-for-all/teen/missing-data-puzzlee", nextGameId: null };
  ...
  const questions = [
    {
      id: 1,
      text: "AI was trained with wrong labels and calls an apple a banana. What should be fixed?",
      options: [
        { 
          id: "labels", 
          text: "Data Labels", 
    ...
    {
      id: 4,
      text: "How can we correct AI misclassification?",
      options: [
        { 
          id: "improve", 
  ...
  useEffect(() => {
    if (showResult) {
      console.log(`🎮 Bad Data Story game completed! Score: ${finalScore}/${questions.length}, gameId: ${gameId}, nextGamePath: ${nextGamePath}, nextGameId: ${nextGameId}`);
            ...
            {finalScore >= 3 ? (
              <div>
                ...
                <p className="text-white/90 text-base md:text-lg mb-4">
                  ...
                  You understand that clean data is essential—garbage in, garbage out!
                </p>
                <div className="bg-gradient-to-r from-yellow-500 to-orange-500 text-white py-2 md:py-3 px-4 md:px-6 rounded-full inline-flex items-center gap-2 mb-4 text-sm md:text-base">
                  <span>+{coins} Coins</span>
                </div>
                <p className="text-white/80 text-sm md:text-base">
                  Well done! Clean data is essential—garbage in, garbage out. 📊🤖
                </p>
              </div>
            ) : (
              <div>
                ...
                <p className="text-white/90 text-base md:text-lg mb-4">
                  You got {finalScore} out of {questions.length} questions correct.
                  Remember: Wrong labels confuse AI.
                </p>
                <button
                ...
                <p className="text-white/80 text-xs md:text-sm">
                  Try to choose the option that shows how data quality directly affects AI performance.
                </p>
              </div>
            )}
          </div>
