...
const DataDiversityStory = () => {
  ...
  const { nextGamePath, nextGameId } = useMemo(() => {
    ...
    try {
      ...
      if (currentGame && currentGame.index !== undefined) {
        ...
        return {
          nextGamePath: nextGame ? nextGame.path : "/student/ai-for-all/teen/training-accuracy-game",
    ...
    return { nextGamePath: "/student/ai-for-all/teen/training-accuracy-game", nextGameId: null };
  ...
  const questions = [
    {
      id: 1,
      text: "The AI recognizes boys correctly but fails on girls. What should be done?",
      options: [
        { 
          id: "diverse", 
    ...
    {
      id: 2,
      text: "AI identifies smiles better on boys. How to fix?",
      options: [
        { 
          id: "include", 
  ...
  useEffect(() => {
    if (showResult) {
      console.log(`🎮 Data Diversity Story game completed! Score: ${finalScore}/${questions.length}, gameId: ${gameId}, nextGamePath: ${nextGamePath}, nextGameId: ${nextGameId}`);
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-xl md:rounded-2xl p-6 md:p-8 border border-white/20 text-center flex-1 flex flex-col justify-center">
            {finalScore >= 3 ? (
              <div>
                ...
                <h3 className="text-xl md:text-2xl font-bold text-white mb-4">Bias Corrected!</h3>
                <p className="text-white/90 text-base md:text-lg mb-4">
                  You got {finalScore} out of {questions.length} questions correct!
                  You understand that adding diverse data ensures AI works fairly across all genders and ages!
                </p>
                <div className="bg-gradient-to-r from-yellow-500 to-orange-500 text-white py-2 md:py-3 px-4 md:px-6 rounded-full inline-flex items-center gap-2 mb-4 text-sm md:text-base">
                  <span>+{coins} Coins</span>
                </div>
                <p className="text-white/80 text-sm md:text-base">
                  Great! Adding diverse data ensures AI works fairly across all genders and ages. 🌏🤖
                </p>
              </div>
            ) : (
              <div>
                ...
                <p className="text-white/90 text-base md:text-lg mb-4">
                  You got {finalScore} out of {questions.length} questions correct.
                  Remember, AI can be biased if data isn't diverse.
                </p>
                <button
                ...
                <p className="text-white/80 text-xs md:text-sm">
                  Try to choose the option that shows how data diversity creates fairer AI systems.
                </p>
              </div>
            )}
          </div>
