...
const DataCleaningPuzzle = () => {
  ...
  const correctMatches = [
    { leftId: 1, rightId: 1 }, // Clear Apple → Keep (left 1st, right 3rd)
    { leftId: 2, rightId: 2 }, // Blurry → Remove (left 2nd, right 1st)
    { leftId: 3, rightId: 3 }, // Complete → Train With (left 3rd, right 5th)
    { leftId: 4, rightId: 4 }, // Incomplete → Fix (left 4th, right 2nd)
    { leftId: 5, rightId: 5 }  // Accurate → Use (left 5th, right 4th)
  ];
  ...
  useEffect(() => {
    if (showResult) {
      console.log(`🎮 Data Cleaning Puzzle game completed! Score: ${finalScore}/${leftItems.length}, gameId: ${gameId}`);
    }
  }, [showResult, finalScore, gameId, leftItems.length]);

  return (
    <GameShell
      title="Puzzle: Data Cleaning"
      score={coins}
      subtitle={showResult ? "Game Complete!" : "Match data items with their actions"}
        ...
        {!showResult ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-xl md:rounded-2xl p-4 md:p-6 border border-white/20">
              <h3 className="text-lg md:text-xl font-bold text-white mb-3 md:mb-4 text-center">Data Items</h3>
            ...
            {finalScore >= 3 ? (
              <div>
                ...
                <p className="text-white/90 text-base md:text-lg mb-4">
                  You correctly matched {finalScore} out of {leftItems.length} data items!
                  You understand how to clean data for AI training!
                </p>
              ...
              <div>
                ...
                <p className="text-white/90 text-base md:text-lg mb-4">
                  You matched {finalScore} out of {leftItems.length} data items correctly.
                  Remember, clean data helps AI learn better!
                </p>
                <button
                ...
                <p className="text-white/80 text-xs md:text-sm">
                  Try to match each data item with its appropriate action.
                </p>
              </div>
            )}
          </div>
