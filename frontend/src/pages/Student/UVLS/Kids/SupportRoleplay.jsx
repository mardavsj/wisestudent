...
const SupportRoleplay = () => {
  ...
  const gameData = useMemo(() => getGameDataById("uvls-kids-28"), []);
  const gameId = gameData?.id || "uvls-kids-28";
  
  // Ensure gameId is always set correctly
  if (!gameData || !gameData.id) {
    console.warn("Game data not found for SupportRoleplay, using fallback ID");
  }
  
  // Get coinsPerLevel, totalCoins, and totalXp from game category data, fallback to location.state, then defaults
  const coinsPerLevel = gameData?.coins || location.state?.coinsPerLevel || 1;
  const totalCoins = gameData?.coins || location.state?.totalCoins || 1;
  const totalXp = gameData?.xp || location.state?.totalXp || 1;
