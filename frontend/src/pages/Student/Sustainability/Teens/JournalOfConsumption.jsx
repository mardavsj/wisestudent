...
const JournalOfConsumption = () => {
  const location = useLocation();
  
  const gameData = getGameDataById("sustainability-teens-32");
  const gameId = gameData?.id || "sustainability-teens-32";
  
  if (!gameData || !gameData.id) {
    console.warn("Game data not found for JournalOfConsumption, using fallback ID");
  }
  ...
  useEffect(() => {
    if (gameFinished) {
      console.log(`🎮 Journal of Consumption game completed! Score: ${coins}, gameId: ${gameId}, nextGamePath: ${nextGamePath}, nextGameId: ${nextGameId}`);
  ...
  const stages = [
    { question: 'Write: "My reflection on my last 5 purchases and how many were necessities versus wants is ___."', minLength: 10 },
    { question: 'Write: "My thoughts on how I could reduce the environmental impact of my purchases are ___."', minLength: 10 },
    { question: 'Write: "My ideas for sustainable alternatives for my future purchases are ___."', minLength: 10 },
    { question: 'Write: "My perspective on how I balance buying quality items versus buying more quantity is ___."', minLength: 10 },
    { question: 'Write: "My goals for more conscious consumption in the next month are ___."', minLength: 10 }
  ];
  ...
  useEffect(() => {
    if (gameFinished) {
      console.log(`🎮 Journal of Consumption game completed! Score: ${coins}/${stages.length}, gameId: ${gameId}`);
    }
...
export default JournalOfConsumption;