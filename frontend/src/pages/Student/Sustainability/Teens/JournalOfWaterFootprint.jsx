...
const JournalOfWaterFootprint = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const gameData = getGameDataById("sustainability-teens-52");
  const gameId = gameData?.id || "sustainability-teens-52";
  ...
  useEffect(() => {
    if (gameFinished) {
      console.log(`🎮 Journal of Water Footprint game completed! Score: ${coins}, gameId: ${gameId}, nextGamePath: ${nextGamePath}, nextGameId: ${nextGameId}`);
  ...
  const stages = [
    { question: 'Write: "My daily water usage includes ___."', minLength: 10 },
    { question: 'Write: "One way I can reduce my water footprint is ___."', minLength: 10 },
    { question: 'Write: "The most water-intensive activity in my home is ___."', minLength: 10 },
    { question: 'Write: "My family could save water by ___."', minLength: 10 },
    { question: 'Write: "I commit to reducing my water footprint by ___."', minLength: 10 }
  ];
  ...
  useEffect(() => {
    if (gameFinished) {
      console.log(`🎮 Journal of Water Footprint game completed! Score: ${coins}/${stages.length}, gameId: ${gameId}`);
    }
...
export default JournalOfWaterFootprint;