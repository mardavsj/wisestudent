...
const questions = [
  {
    id: 1,
    text: "Which action protects habitats?",
    options: [
  ...
  {
    id: 3,
    text: "Which practice supports habitat health?",
    options: [
      { id: 'a', text: " Clear cutting forests", emoji: "🪵", isCorrect: false },
      { id: 'b', text: " Creating wildlife corridors", emoji: "🦌", isCorrect: true },
...
const ReflexHabitatProtector = () => {
  const location = useLocation();
  
  const gameData = getGameDataById("sustainability-teens-64");
  const gameId = gameData?.id || "sustainability-teens-64";
  ...
  useEffect(() => {
    if (gameState === "finished") {
      console.log(`🎮 Reflex Habitat Protector game completed! Score: ${score}/${TOTAL_ROUNDS}, gameId: ${gameId}, nextGamePath: ${nextGamePath}, nextGameId: ${nextGameId}`);
  ...
  return (
    <GameShell
      ...
      nextGamePathProp="/student/sustainability/teens/badge-biodiversity-champion"
      nextGameIdProp="sustainability-teens-65">
      <div className="space-y-8">
        {gameState === "ready" && (
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-8 border border-white/20 text-center">
            <h3 className="text-2xl font-bold text-white mb-4">Get Ready!</h3>
            <p className="text-white/80 mb-6 text-lg">Choose the habitat-protecting action quickly! You have {ROUND_TIME} seconds per round</p>
            <button
              onClick={startGame}
...
export default ReflexHabitatProtector;