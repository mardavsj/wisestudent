...
const questions = [
  ...
  {
    id: 4,
    text: "What's an innovative energy solution?",
    options: [
...
const ReflexEnergyTech = () => {
  const location = useLocation();
  
  const gameData = getGameDataById("sustainability-teens-44");
  const gameId = gameData?.id || "sustainability-teens-44";
  ...
  useEffect(() => {
    if (gameState === "finished") {
      console.log(`🎮 Reflex Energy Tech game completed! Score: ${score}/${TOTAL_ROUNDS}, gameId: ${gameId}, nextGamePath: ${nextGamePath}, nextGameId: ${nextGameId}`);
  ...
  return (
    <GameShell
      ...
      nextGamePathProp="/student/sustainability/teens/badge-energy-innovator"
      nextGameIdProp="sustainability-teens-45">
      <div className="space-y-8">
        {gameState === "ready" && (
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-8 border border-white/20 text-center">
            <h3 className="text-2xl font-bold text-white mb-4">Get Ready!</h3>
            <p className="text-white/80 mb-6 text-lg">Identify green technologies quickly! You have {ROUND_TIME} seconds per round</p>
            <button
              onClick={startGame}
...
export default ReflexEnergyTech;