...
const JournalOfWasteAudit = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const gameData = getGameDataById("sustainability-teens-17");
  const gameId = gameData?.id || "sustainability-teens-17";
  ...
  useEffect(() => {
    if (gameFinished) {
      console.log(`🎮 Journal of Waste Audit game completed! Score: ${coins}/${stages.length}, gameId: ${gameId}`);
    }
  ...
  return (
    <GameShell
      ...
      nextGamePathProp="/student/sustainability/teens/simulation-recycling-program"
      nextGameIdProp="sustainability-teens-18">
...
export default JournalOfWasteAudit;
