...
const questions = [
  {
    id: 1,
    text: "Which item is zero-waste friendly?",
    options: [
  ...
  {
    id: 4,
    text: "Which container is reusable?",
    options: [
  ...
  {
    id: 5,
    text: "Which item reduces waste?",
    options: [
      { id: 'a', text: " Reusable Lunchbox", emoji: "🍱", isCorrect: true },
      { id: 'b', text: " Single-use Bottle", emoji: "🥤", isCorrect: false },
      { id: 'c', text: " Disposable Plates", emoji: "🍽️", isCorrect: false },
      { id: 'd', text: " Plastic Utensils", emoji: "🥢", isCorrect: false }
    ]
  }
];

const ReflexZeroWaste = () => {
  const location = useLocation();
  const gameData = getGameDataById("sustainability-teens-13");
  const gameId = gameData?.id || "sustainability-teens-13";
  ...
  useEffect(() => {
    if (gameState === "finished") {
      console.log(`🎮 Reflex Zero Waste game completed! Score: ${score}/${TOTAL_ROUNDS}, gameId: ${gameId}, nextGamePath: ${nextGamePath}, nextGameId: ${nextGameId}`);
      ...
      <div className="space-y-8">
        {gameState === "ready" && (
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-8 border border-white/20 text-center">
            ...
            <p className="text-white/80 mb-6 text-lg">Choose the zero-waste item quickly! You have {ROUND_TIME} seconds per round</p>
            <button
              onClick={startGame}
...
export default ReflexZeroWaste;