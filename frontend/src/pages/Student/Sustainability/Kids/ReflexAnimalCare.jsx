...
const questions = [
  {
    id: 1,
    text: "Tap for 'Help Animals,' avoid 'Harm Animals'",
    options: [
  ...
  },
  {
    ...
    options: [
      { id: 'b', text: " Make noise", emoji: "🔊", isCorrect: false },
      { id: 'c', text: " Litter", emoji: "🗑️", isCorrect: false },
      { id: 'd', text: " Disturb nests", emoji: "❌", isCorrect: false },
  ...
  {
    id: 5,
    text: "How can you help animals in winter?",
    options: [
      { id: 'b', text: " Leave food", emoji: "🍎", isCorrect: false },
      { id: 'c', text: " Build shelters", emoji: "🏠", isCorrect: false },
      { id: 'a', text: " Provide water", emoji: "💧", isCorrect: true },
      { id: 'd', text: " Ignore them", emoji: "😶", isCorrect: false }
    ]
  }
];

const ReflexAnimalCare = () => {
  const navigate = useNavigate();
  const location = useLocation();
  
  // Get game data from game category folder (source of truth)
  const gameId = "sustainability-kids-58";
  ...
  useEffect(() => {
    if (gameState === "finished") {
      console.log(`🎮 Reflex Animal Care game completed! Score: ${score}/${TOTAL_ROUNDS}, gameId: ${gameId}, nextGamePath: ${nextGamePath}, nextGameId: ${nextGameId}`);
  ...
  return (
    <GameShell
      ...
      nextGamePathProp="/student/sustainability/kids/puzzle-animal-homes"
      nextGameIdProp="sustainability-kids-59">
      <div className="space-y-8">
        {gameState === "ready" && (
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-8 border border-white/20 text-center">
            <h3 className="text-2xl font-bold text-white mb-4">Get Ready!</h3>
            <p className="text-white/80 mb-6 text-lg">Choose the animal care action quickly! You have {ROUND_TIME} seconds per round</p>
            <button
              onClick={startGame}
...
export default ReflexAnimalCare;