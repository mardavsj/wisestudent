...
const WaterTapStory = () => {
  ...
  const gameData = getGameDataById("sustainability-kids-15");
  const gameId = gameData?.id || "sustainability-kids-15";
  
  // Ensure gameId is always set correctly
  if (!gameData || !gameData.id) {
    console.warn("Game data not found for WaterTapStory, using fallback ID");
  }
  ...
  useEffect(() => {
    if (showResult) {
      console.log(`🎮 Water Tap Story game completed! Score: ${finalScore}, gameId: ${gameId}, nextGamePath: ${nextGamePath}, nextGameId: ${nextGameId}`);
  ...
  const questions = [
    {
      id: 1,
      text: "You see a tap dripping. What should you do?",
      options: [
    ...
    {
      id: 2,
      text: "Why is saving water important?",
      options: [
        { id: "nothing", text: "It doesn't matter", emoji: "😐", isCorrect: false },
        { id: "precious", text: "Water is precious", emoji: "💧", isCorrect: true },
        { id: "waste", text: "We can waste it", emoji: "💦", isCorrect: false }
      ]
    },
    {
      id: 3,
  ...
  return (
    <GameShell
      ...
      nextGamePathProp="/student/sustainability/kids/poster-save-water"
      nextGameIdProp="sustainability-kids-16">
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-xl md:rounded-2xl p-6 md:p-8 border border-white/20 text-center flex-1 flex flex-col justify-center">
            {finalScore >= 3 ? (
              <div>
                ...
                <h3 className="text-xl md:text-2xl font-bold text-white mb-4">Water Conservation Hero!</h3>
                <p className="text-white/90 text-base md:text-lg mb-4">
                  You got {finalScore} out of {questions.length} questions correct!
                  You understand how to save water and protect this precious resource!
                </p>
                <div className="bg-gradient-to-r from-yellow-500 to-orange-500 text-white py-2 md:py-3 px-4 md:px-6 rounded-full inline-flex items-center gap-2 mb-4 text-sm md:text-base">
                  <span>+{coins} Coins</span>
                </div>
                <p className="text-white/80 text-sm md:text-base">
                  Great job! You know how to conserve water in your daily life!
                </p>
              </div>
            ) : (
              <div>
                ...
                <p className="text-white/90 text-base md:text-lg mb-4">
                  You got {finalScore} out of {questions.length} questions correct.
                  Remember, saving water helps protect our planet!
                </p>
                <button
                ...
                <p className="text-white/80 text-xs md:text-sm">
                  Try to choose the option that shows water-saving behaviors.
                </p>
              </div>
            )}
          </div>
