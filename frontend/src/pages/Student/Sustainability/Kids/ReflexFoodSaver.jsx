...
const ReflexFoodSaver = () => {
  const navigate = useNavigate();
  const location = useLocation();
  
  // Get game data from game category folder (source of truth)
  const gameId = "sustainability-kids-38";
  ...
  useEffect(() => {
    if (gameState === "finished") {
      console.log(`🎮 Reflex Food Saver game completed! Score: ${score}/${TOTAL_ROUNDS}, gameId: ${gameId}, nextGamePath: ${nextGamePath}, nextGameId: ${nextGameId}`);
  ...
  return (
    <GameShell
      ...
      nextGamePathProp="/student/sustainability/kids/puzzle-compost-items"
      nextGameIdProp="sustainability-kids-39">
      <div className="space-y-8">
        {gameState === "ready" && (
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-8 border border-white/20 text-center">
            <h3 className="text-2xl font-bold text-white mb-4">Get Ready!</h3>
            <p className="text-white/80 mb-6 text-lg">Choose the food-saving action quickly! You have {ROUND_TIME} seconds per round</p>
            <button
              onClick={startGame}
...
export default ReflexFoodSaver;