...
const JournalOfDoctorVisits = () => {
  const navigate = useNavigate();
  const location = useLocation();
  
  // Get game data from game category folder (source of truth)
  const gameId = "health-female-teen-77";
  ...
  const stages = [
    {
      question: 'Write: "One time I visited a doctor for ___.',
      minLength: 10,
    },
    {
      question: 'Write: "I felt ___ during my last doctor visit.',
      minLength: 10,
    },
    {
      question: 'Write: "Something I learned about my health is ___.',
      minLength: 10,
    },
    {
      question: 'Write: "A question I would ask a doctor is ___.',
      minLength: 10,
    },
    {
      question: 'Write: "One way I can take better care of my health is ___.',
      minLength: 10,
    },
  ];
  ...
  useEffect(() => {
    if (showResult) {
      console.log(`🎮 Journal of Doctor Visits game completed! Score: ${finalScore}/${stages.length}, gameId: ${gameId}`);
    }
  }, [showResult, finalScore, gameId, stages.length]);

  const handleNext = () => {
    navigate("/student/health-female/teens/simulation-clinic-visit");
  };

  return (
    <GameShell
      title="Journal of Doctor Visits"
      subtitle={!showResult ? `Question ${currentStage + 1} of ${stages.length}: Reflect on your healthcare experiences!` : "Journal Complete!"}
      ...
      nextGamePathProp="/student/health-female/teens/simulation-clinic-visit"
      nextGameIdProp="health-female-teen-78">
      <div className="min-h-[calc(100vh-200px)] flex flex-col justify-center text-center text-white space-y-6 md:space-y-8 max-w-4xl mx-auto px-4 py-4">
        {!showResult && stages[currentStage] && (
          <div className="bg-white/10 backdrop-blur-md p-6 md:p-8 rounded-xl md:rounded-2xl border border-white/20">
            <PenSquare className="mx-auto mb-4 w-8 h-8 md:w-10 md:h-10 text-blue-300" />
            ...
            <button
              ...
              className={`mt-4 px-6 md:px-8 py-3 md:py-4 rounded-full text-base md:text-lg font-semibold transition-transform ${
                entry.trim().length >= stages[currentStage].minLength && !showResult
                  ? 'bg-blue-500 hover:bg-blue-600 hover:scale-105 text-white cursor-pointer'
        ...
        {showResult && (
          <div className="bg-white/10 backdrop-blur-md p-6 md:p-8 rounded-xl md:rounded-2xl border border-white/20 text-center">
            ...
            <h2 className="text-2xl font-bold text-white mb-6">Doctor Visits Journal Complete!</h2>
            
            <div className="bg-gradient-to-r from-blue-500/20 to-indigo-500/20 rounded-xl p-4 border border-blue-400/30 mb-6">
              <p className="text-blue-300 font-bold">
                🎉 Great job! You've completed your doctor visits journal!
              </p>
              <p className="text-blue-300 mt-2">
                Recording your thoughts about healthcare helps you stay informed!
              </p>
            </div>
            ...
            >
              Continue to Clinic Visit Simulation
            </button>
          </div>
        )}
      </div>
