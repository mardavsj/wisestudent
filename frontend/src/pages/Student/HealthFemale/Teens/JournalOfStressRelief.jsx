...
const JournalOfStressRelief = () => {
  ...
  const gameId = "health-female-teen-57";
  ...
  const stages = [
    {
      question: 'Write: "One way I manage stress is ___.',
      minLength: 10,
    },
    {
      question: 'Write: "A relaxing activity I enjoy is ___.',
      minLength: 10,
    },
    {
      question: 'Write: "When I feel overwhelmed, I ___.',
      minLength: 10,
    },
    {
      question: 'Write: "A person I can talk to about stress is ___.',
      minLength: 10,
    },
    {
      question: 'Write: "Something that makes me feel confident is ___.',
      minLength: 10,
    },
  ];
  ...
  useEffect(() => {
    if (showResult) {
      console.log(`🎮 Journal of Stress Relief game completed! Score: ${finalScore}/${stages.length}, gameId: ${gameId}`);
    }
  }, [showResult, finalScore, gameId, stages.length]);

  const handleNext = () => {
    navigate("/student/health-female/teens/simulation-teen-stress-day");
  };

  return (
    <GameShell
      title="Journal of Stress Relief"
      subtitle={!showResult ? `Question ${currentStage + 1} of ${stages.length}: Reflect on your stress management!` : "Journal Complete!"}
      ...
      nextGamePathProp="/student/health-female/teens/simulation-teen-stress-day"
      nextGameIdProp="health-female-teen-58">
      <div className="min-h-[calc(100vh-200px)] flex flex-col justify-center text-center text-white space-y-6 md:space-y-8 max-w-4xl mx-auto px-4 py-4">
        {!showResult && stages[currentStage] && (
          <div className="bg-white/10 backdrop-blur-md p-6 md:p-8 rounded-xl md:rounded-2xl border border-white/20">
            <PenSquare className="mx-auto mb-4 w-8 h-8 md:w-10 md:h-10 text-teal-300" />
            ...
            <button
              ...
              className={`mt-4 px-6 md:px-8 py-3 md:py-4 rounded-full text-base md:text-lg font-semibold transition-transform ${
                entry.trim().length >= stages[currentStage].minLength && !showResult
                  ? 'bg-teal-500 hover:bg-teal-600 hover:scale-105 text-white cursor-pointer'
        ...
        {showResult && (
          <div className="bg-white/10 backdrop-blur-md p-6 md:p-8 rounded-xl md:rounded-2xl border border-white/20 text-center">
            ...
            <h2 className="text-2xl font-bold text-white mb-6">Stress Relief Journal Complete!</h2>
            
            <div className="bg-gradient-to-r from-teal-500/20 to-cyan-500/20 rounded-xl p-4 border border-teal-400/30 mb-6">
              <p className="text-teal-300 font-bold">
                🎉 Great job! You've completed your stress relief journal!
              </p>
              <p className="text-teal-300 mt-2">
                Recording your thoughts about stress management helps you stay balanced!
              </p>
            </div>
            ...
            >
              Continue to Stress Day Simulation
            </button>
          </div>
        )}
      </div>
