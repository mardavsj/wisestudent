...
const JournalOfSelfRespect = () => {
  const navigate = useNavigate();
  const location = useLocation();
  
  // Get game data from game category folder (source of truth)
  const gameId = "health-female-teen-67";
  ...
  const stages = [
    {
      question: 'Write: "One thing I like about myself is ___.',
      minLength: 10,
    },
    {
      question: 'Write: "A skill or talent I\'m proud of is ___.',
      minLength: 10,
    },
    {
      question: 'Write: "A kind thing I\'ve done for someone else is ___.',
      minLength: 10,
    },
    {
      question: 'Write: "A challenge I\'ve overcome is ___.',
      minLength: 10,
    },
    {
      question: 'Write: "Something I\'m grateful for about my life is ___.',
      minLength: 10,
    },
  ];
  ...
  useEffect(() => {
    if (showResult) {
      console.log(`🎮 Journal of Self-Respect game completed! Score: ${finalScore}/${stages.length}, gameId: ${gameId}`);
    }
  }, [showResult, finalScore, gameId, stages.length]);

  const handleNext = () => {
    navigate("/student/health-female/teens/simulation-peer-test");
  };

  return (
    <GameShell
      title="Journal of Self-Respect"
      subtitle={!showResult ? `Question ${currentStage + 1} of ${stages.length}: Reflect on your self-respect!` : "Journal Complete!"}
      ...
      nextGamePathProp="/student/health-female/teens/simulation-peer-test"
      nextGameIdProp="health-female-teen-68">
      <div className="min-h-[calc(100vh-200px)] flex flex-col justify-center text-center text-white space-y-6 md:space-y-8 max-w-4xl mx-auto px-4 py-4">
        ...
        {showResult && (
          <div className="bg-white/10 backdrop-blur-md p-6 md:p-8 rounded-xl md:rounded-2xl border border-white/20 text-center">
            ...
            <h2 className="text-2xl font-bold text-white mb-6">Self-Respect Journal Complete!</h2>
            ...
            <div className="bg-gradient-to-r from-purple-500/20 to-indigo-500/20 rounded-xl p-4 border border-purple-400/30 mb-6">
              <p className="text-purple-300 font-bold">
                🎉 Great job! You've completed your self-respect journal!
              </p>
              <p className="text-purple-300 mt-2">
                Recording your thoughts about self-respect helps you build confidence!
              </p>
            </div>
            ...
            >
              Continue to Peer Test Simulation
            </button>
          </div>
        )}
      </div>
...
export default JournalOfSelfRespect;