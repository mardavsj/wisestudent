...
const JournalOfTeenHabits = () => {
  ...
  const gameId = "health-female-teen-97";
  ...
  const stages = [
    {
      question: 'Write: "One habit I built as a teen is ___.',
      minLength: 10,
    },
    {
      question: 'Write: "A healthy routine I follow daily is ___.',
      minLength: 10,
    },
    {
      question: 'Write: "The biggest challenge in maintaining good habits is ___.',
      minLength: 10,
    },
    {
      question: 'Write: "A strategy that helps me stay consistent is ___.',
      minLength: 10,
    },
    {
      question: 'Write: "How my habits have improved my life is ___.',
      minLength: 10,
    },
  ];
  ...
  useEffect(() => {
    if (showResult) {
      console.log(`🎮 Journal of Teen Habits game completed! Score: ${finalScore}/${stages.length}, gameId: ${gameId}`);
    }
  }, [showResult, finalScore, gameId, stages.length]);

  const handleNext = () => {
    navigate("/student/health-female/teens/simulation-time-balance");
  };

  return (
    <GameShell
      title="Journal of Teen Habits"
      subtitle={!showResult ? `Question ${currentStage + 1} of ${stages.length}: Reflect on your healthy habits!` : "Journal Complete!"}
      ...
      nextGamePathProp="/student/health-female/teens/simulation-time-balance"
      nextGameIdProp="health-female-teen-98">
      <div className="min-h-[calc(100vh-200px)] flex flex-col justify-center text-center text-white space-y-6 md:space-y-8 max-w-4xl mx-auto px-4 py-4">
        ...
        {showResult && (
          <div className="bg-white/10 backdrop-blur-md p-6 md:p-8 rounded-xl md:rounded-2xl border border-white/20 text-center">
            ...
            <h2 className="text-2xl font-bold text-white mb-6">Teen Habits Journal Complete!</h2>
            
            <div className="bg-gradient-to-r from-green-500/20 to-emerald-500/20 rounded-xl p-4 border border-green-400/30 mb-6">
              <p className="text-green-300 font-bold">
                🎉 Great job! You've completed your teen habits journal!
              </p>
              <p className="text-green-300 mt-2">
                Recording your thoughts about healthy habits helps you build better routines!
              </p>
            </div>
            ...
            >
              Continue to Time Balance Simulation
            </button>
          </div>
        )}
      </div>
