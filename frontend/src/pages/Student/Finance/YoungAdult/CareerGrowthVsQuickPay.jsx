...
const CareerGrowthVsQuickPay = () => {
  ...
  const gameContent = t(
    "financial-literacy.young-adult.career-growth-vs-quick-pay",
    { returnObjects: true }
  );
  ...
  return (
    <GameShell
      title={gameContent?.title || "Career Growth vs Quick Pay"}
      subtitle={subtitle}
      ...
      <div className="space-y-5 text-white">
        <div className="bg-white/10 border border-white/20 rounded-3xl p-8 shadow-2xl max-w-4xl mx-auto">
          <div className="flex justify-between items-center mb-4 text-sm uppercase tracking-[0.3em] text-white/60">
            ...
            <span>{gameContent?.scenarioValue || "Career Growth"}</span>
          </div>
              ...
              {showResult && (
                <>
                  ...
                  <p className="text-sm text-white/70">
                    ...
                    <strong>{gameContent?.skillName || "Career growth strategy"}</strong>
                  </p>
                  <p className="text-base text-green-400 font-semibold mt-4">
                    {gameContent?.outcomeLine ||
                      "Outcome: Learning-first choices build resilient careers."}
                  </p>
                  {!hasPassed && (
          ...
          <div className="bg-white/5 border border-white/20 rounded-3xl p-6 shadow-xl max-w-4xl mx-auto space-y-3">
            ...
            <p className="text-base text-green-400 font-semibold">
              {gameContent?.outcomeLineShort ||
                "Learning-first choices build resilient careers."}
            </p>
            {!hasPassed && (
              <p className="text-xs text-amber-300">
                {gameContent?.fullRewardHint
