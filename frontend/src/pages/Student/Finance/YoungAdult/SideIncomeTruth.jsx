...
const SideIncomeTruth = () => {
  ...
  const gameContent = t(
    "financial-literacy.young-adult.side-income-truth",
    { returnObjects: true }
  );
  ...
  const subtitle =
    gameContent?.subtitleProgress?.replace("{{current}}", Math.min(currentStage + 1, totalStages))
  ...
  return (
    <GameShell
      title={gameContent?.title || "Side Income Truth"}
      subtitle={subtitle}
      ...
      <div className="space-y-5 text-white">
        <div className="bg-white/10 border border-white/20 rounded-3xl p-8 shadow-2xl max-w-4xl mx-auto">
          <div className="flex justify-between items-center mb-4 text-sm uppercase tracking-[0.3em] text-white/60">
            ...
            <span>{gameContent?.scenarioValue || "Side Income"}</span>
          </div>
        ...
        {showResult && (
          <div className="bg-white/5 border border-white/20 rounded-3xl p-6 shadow-xl max-w-4xl mx-auto space-y-3">
            ...
            <p className="text-sm text-white/70">
              ...
              <strong>{gameContent?.skillName || "Side income strategy"}</strong>
            </p>
