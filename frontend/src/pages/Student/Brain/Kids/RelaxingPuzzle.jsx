...
const RelaxingPuzzle = () => {
  ...
  const gameData = getGameDataById("brain-kids-34");
  const gameId = gameData?.id || "brain-kids-34";
  
  const gameContent = t("brain-health.kids.relaxing-puzzle", { returnObjects: true });
  
  // Ensure gameId is always set correctly
  if (!gameData || !gameData.id) {
    console.warn("Game data not found for RelaxingPuzzle, using fallback ID");
  }
  ...
  const correctMatches = [
    { leftId: 1, rightId: 1 }, // Yoga → Calm
    { leftId: 2, rightId: 2 }, // Music → Relax
    { leftId: 3, rightId: 3 }, // Deep Breathing → Peace
    { leftId: 4, rightId: 4 }, // Nature Walk → Tranquil
    { leftId: 5, rightId: 5 }  // Reading → Serene
  ];
  ...
  return (
    <GameShell
      title={gameContent?.title || "Puzzle of Relaxing"}
      subtitle={
        showResult 
          ? gameContent?.subtitleComplete || "Puzzle Complete!" 
          : t("brain-health.kids.relaxing-puzzle.subtitleMatching", {
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
              <div className="flex justify-between items-center mb-4">
                <span className="text-white/80">
                  {t("brain-health.kids.relaxing-puzzle.matchesLabel", {
                    current: matches.length,
                    total: leftItems.length,
                ...
                <span className="text-yellow-400 font-bold">
                  {t("brain-health.kids.relaxing-puzzle.scoreLabel", {
                    current: score,
                    total: leftItems.length,
              ...
              <p className="text-white text-lg mb-6 text-center">
                {gameContent?.instruction || "Match the relaxing activities with their effects!"}
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                  <h3 className="text-white font-bold text-center mb-4">
                    {gameContent?.leftTitle || "Relaxing Activities"}
                  </h3>
                  {leftItems.map((item) => (
                    <button
                      key={item.id}
                ...
                <div className="space-y-3">
                  <h3 className="text-white font-bold text-center mb-4">
                    {gameContent?.rightTitle || "Relaxation Effects"}
                  </h3>
