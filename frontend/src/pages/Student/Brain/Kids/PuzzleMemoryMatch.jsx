...
const PuzzleMemoryMatch = () => {
  ...
  const gameContent = t("brain-health.kids.puzzle-memory-match", { returnObjects: true });
  ...
  return (
    <GameShell
      title={gameContent?.title || "Puzzle: Memory Match"}
      score={score}
      subtitle={
        showResult 
          ? gameContent?.subtitleComplete || "Game Complete!" 
          : t("brain-health.kids.puzzle-memory-match.subtitleProgress", {
              current: matches.length,
              total: leftItems.length,
              defaultValue: `Match fruits shown earlier (${matches.length}/${leftItems.length} matched)`,
            })
      }
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
              <h3 className="text-xl font-bold text-white mb-4 text-center">
                {gameContent?.fruitsShownTitle || "Fruits Shown"}
              </h3>
            ...
            <div className="flex flex-col items-center justify-center">
              <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 text-center w-full">
                <p className="text-white/80 mb-4 h-12 flex items-center justify-center">
                  {selectedLeft 
                    ? t("brain-health.kids.puzzle-memory-match.selectedLabel", {
                        name: selectedLeft.name,
                        defaultValue: `Selected: ${selectedLeft.name}`,
                      })
                    : gameContent?.selectAFruit || "Select a fruit"}
                </p>
                <button
                  onClick={handleMatch}
                ...
                <div className="mt-4 text-white/80">
                  <p>
                    {t("brain-health.kids.puzzle-memory-match.scoreLabel", {
                      score,
                      total: leftItems.length,
                  ...
                  <p>
                    {t("brain-health.kids.puzzle-memory-match.matchedLabel", {
                      current: matches.length,
                      total: leftItems.length,
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
              <h3 className="text-xl font-bold text-white mb-4 text-center">
                {gameContent?.matchTheseTitle || "Match These"}
              </h3>
              <div className="space-y-4">
            ...
            {score >= 3 ? (
              <div>
                ...
                <h3 className="text-2xl font-bold text-white mb-4">
                  {gameContent?.greatMemoryTitle || "Great Memory!"}
                </h3>
                <p className="text-white/90 text-lg mb-4">
                  {t("brain-health.kids.puzzle-memory-match.greatMemoryDescription", {
                    score,
                    total: leftItems.length,
                    defaultValue: `You correctly matched ${score} out of ${leftItems.length} fruits! Your memory is working well!`,
                  })}
                </p>
                <div className="bg-gradient-to-r from-yellow-500 to-orange-500 text-white py-3 px-6 rounded-full inline-flex items-center gap-2 mb-4">
                  <span>
                    {t("brain-health.kids.puzzle-memory-match.coinsAwarded", {
                      score,
                ...
                <p className="text-white/80">
                  {gameContent?.rememberedFruits || "You remembered the fruits: Apple, Banana, Orange, Grape, and Mango!"}
                </p>
              </div>
            ) : (
              <div>
                <div className="text-5xl mb-4">😔</div>
                <h3 className="text-2xl font-bold text-white mb-4">
                  {gameContent?.keepPracticingTitle || "Keep Practicing!"}
                </h3>
                <p className="text-white/90 text-lg mb-4">
                  {t("brain-health.kids.puzzle-memory-match.keepPracticingDescription", {
                    score,
                    total: leftItems.length,
                    defaultValue: `You matched ${score} out of ${leftItems.length} fruits correctly. Remember, practice helps improve your memory!`,
                  })}
                </p>
                <p className="text-white/80 text-sm">
                  {gameContent?.tryAgainPrompt || "Try to match each fruit with its pair. The fruits were: Apple, Banana, Orange, Grape, and Mango!"}
                </p>
              </div>
            )}
          </div>
