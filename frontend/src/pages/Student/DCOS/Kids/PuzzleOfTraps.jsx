...
const PuzzleOfTraps = () => {
  ...
  const gameData = getGameDataById("dcos-kids-46");
  const gameId = gameData?.id || "dcos-kids-46";
  
  // Ensure gameId is always set correctly
  if (!gameData || !gameData.id) {
    console.warn("Game data not found for PuzzleOfTraps, using fallback ID");
  }
  ...
  return (
    <GameShell
      ...
      score={score}
      subtitle={showResult ? "Puzzle Complete!" : `Match items with their safety status (${matches.length}/${leftItems.length} matched)`}
          ...
          <div className="space-y-6">
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
              ...
              <p className="text-white/90 text-center mb-6">
                Select an item from the left and its safety status from the right, then click "Match"
              </p>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                {/* Left column - Items */}
                <div>
                  <h4 className="text-lg font-semibold mb-4 text-white text-center">Items</h4>
