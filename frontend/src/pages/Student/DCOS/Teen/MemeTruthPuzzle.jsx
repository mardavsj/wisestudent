...
const MemeTruthPuzzle = () => {
  const location = useLocation();
  
  // Get game data from game category folder (source of truth)
  const gameId = "dcos-teen-35";
  ...
  useEffect(() => {
    if (showResult) {
      console.log(`🎮 Meme Truth Puzzle game completed! Score: ${finalScore}/${leftItems.length}, gameId: ${gameId}, nextGamePath: ${nextGamePath}, nextGameId: ${nextGameId}`);
  ...
  return (
    <GameShell
      ...
      score={coins}
      subtitle={showResult ? "Game Complete!" : "Match content types to their nature"}
        ...
        {!showResult ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
              <h3 className="text-xl font-bold text-white mb-4 text-center">Content Type</h3>
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
              <h3 className="text-xl font-bold text-white mb-4 text-center">Nature</h3>
              <div className="space-y-4">
                {rightItems.map(item => {
            ...
            {finalScore >= 3 ? (
              <div>
                ...
                <p className="text-white/90 text-lg mb-4">
                  You correctly matched {finalScore} out of {leftItems.length} content types!
                  You know the difference between jokes and facts!
                </p>
                <div className="bg-gradient-to-r from-yellow-500 to-orange-500 text-white py-3 px-6 rounded-full inline-flex items-center gap-2 mb-4">
                  <span>+{coins} Coins</span>
                </div>
                <p className="text-white/80">
                  Memes are jokes/entertainment, not facts. News should be verified facts from trusted sources. Always verify viral messages and social media posts before believing or sharing them!
                </p>
              </div>
            ) : (
              <div>
                ...
                <p className="text-white/90 text-lg mb-4">
                  You matched {finalScore} out of {leftItems.length} content types correctly.
                  Remember, memes are jokes, news are facts, and always verify social media posts!
                </p>
                <button
                ...
                <p className="text-white/80 text-sm">
                  Try to match each content type with its appropriate nature.
                </p>
              </div>
            )}
          </div>
...
export default MemeTruthPuzzle;
