...
const BankingSafetyPuzzle = () => {
  const location = useLocation();
  
  // Get game data from game category folder (source of truth)
  const gameId = "dcos-teen-47";
  ...
  useEffect(() => {
    if (showResult) {
      console.log(`🎮 Banking Safety Puzzle game completed! Score: ${finalScore}/${leftItems.length}, gameId: ${gameId}, nextGamePath: ${nextGamePath}, nextGameId: ${nextGameId}`);
  ...
  return (
    <GameShell
      ...
      score={coins}
      subtitle={showResult ? "Game Complete!" : "Match banking items to their safety level"}
        ...
        {!showResult ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
              <h3 className="text-xl font-bold text-white mb-4 text-center">Banking Item</h3>
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
              <h3 className="text-xl font-bold text-white mb-4 text-center">Safety Level</h3>
              <div className="space-y-4">
                {rightItems.map(item => {
            ...
            {finalScore >= 3 ? (
              <div>
                ...
                <p className="text-white/90 text-lg mb-4">
                  You correctly matched {finalScore} out of {leftItems.length} banking items!
                  You know about banking safety!
                </p>
                <div className="bg-gradient-to-r from-yellow-500 to-orange-500 text-white py-3 px-6 rounded-full inline-flex items-center gap-2 mb-4">
                  <span>+{coins} Coins</span>
                </div>
                <p className="text-white/80">
                  OTP and bank passwords are PRIVATE - never share them! Bank apps and official bank websites are safe. Fake links are scams. Always use official banking apps and websites, never click suspicious links!
                </p>
              </div>
            ) : (
              <div>
                ...
                <p className="text-white/90 text-lg mb-4">
                  You matched {finalScore} out of {leftItems.length} banking items correctly.
                  Remember, OTP and passwords are private, bank apps are safe, and fake links are scams!
                </p>
                <button
                ...
                <p className="text-white/80 text-sm">
                  Try to match each banking item with its appropriate safety level.
                </p>
              </div>
            )}
          </div>
...
export default BankingSafetyPuzzle;
